...
export default function ManometriaPage() {
  ...
  const addLayer = () => {
    setLayers([...layers, { id: nextId++, rho: "", h: "", direction: "desce" }]);
  };
  ...
  const calculate = () => {
    ...
    steps.push({
      label: "Lei de Stevin",
      formula: "ΔP = ρ × g × h",
      result: "A pressão varia linearmente com a profundidade em um fluido estático.",
    });

    let deltaP = 0;
    layers.forEach((layer, i) => {
      const rho = parseFloat(layer.rho);
      ...
      steps.push({
        label: `Camada ${i + 1} (${layer.direction === "desce" ? "↓ desce" : "↑ sobe"})`,
        formula: `${sign > 0 ? "+" : "−"} ρ × g × h = ${sign > 0 ? "+" : "−"} ${rho} × ${g} × ${h}`,
        result: `${sign > 0 ? "+" : "−"} ${contrib.toFixed(2)} Pa`,
      });
    });

    steps.push({
      label: "Resultado — Diferença de Pressão",
      result: `ΔP = ${deltaP.toFixed(2)} Pa = ${(deltaP / 1000).toFixed(4)} kPa`,
    });
  ...
  return (
    <CalculatorLayout title="Estática dos Fluidos — Manometria">
      <p className="text-xs font-body text-muted-foreground mb-4">
        Adicione as camadas de fluido do manômetro em U. Marque "desce" para colunas que aumentam a pressão e "sobe" para as que diminuem.
      </p>

      <div className="flex flex-col gap-4 mb-6">
        {layers.map((layer, i) => (
          <div key={layer.id} className="border border-foreground p-4 flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              {layers.length > 1 && (
                <button
                  ...
                  className="text-xs font-heading text-destructive cursor-pointer uppercase tracking-wider"
                >
                  Remover
                </button>
              )}
            ...
            <div className="grid grid-cols-3 gap-3">
              <InputField
                ...
                unit="kg/m³"
                value={layer.rho}
                onChange={(val) => updateLayer(layer.id, "rho", val)}
              />
              <InputField
                label="Altura (h)"
                unit="m"
                value={layer.h}
                onChange={(val) => updateLayer(layer.id, "h", val)}
              />
      ...
      <div className="flex gap-3 mb-8">
        <button
          onClick={addLayer}
          className="border border-foreground bg-background text-foreground font-heading text-sm uppercase tracking-wider px-6 py-3 cursor-pointer"
        >
          + Adicionar Camada
        </button>
        <button
          onClick={calculate}
          className="bg-primary text-primary-foreground font-heading text-sm uppercase tracking-wider px-8 py-3 border-none cursor-pointer"
        >
          Calcular
        </button>
      </div>
